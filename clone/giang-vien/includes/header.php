<?php
// giang-vien/includes/header.php
?>
<!DOCTYPE html>
<html lang="vi">
<head>
  ...
  <title>Cổng Thông Tin Giảng Viên — Trường Đại Học HNDA</title>
...
<header class="site-header" style="background: #ffffff; padding: 10px 28px; border-bottom: 2px solid #3b82f6; display: flex; justify-content: space-between; align-items: center;">
  <div class="header-left">
    <a href="index.php" style="text-decoration: none; color: inherit; display: flex; align-items: center; gap: 12px;" title="Về trang chủ Giảng viên">
      ...
      <div>
        ...
        <span style="color: #B91C1C; font-weight: 700; font-size: 11px; display: block; text-transform: uppercase; margin-top: 2px;">CỔNG THÔNG TIN GIẢNG VIÊN</span>
      </div>
    </a>
  </div>
  <div class="header-right" style="display: flex; align-items: center; gap: 14px; position: relative;">
    <div class="user-text" style="text-align: right;">
      <span class="user-name" style="color: #0f172a; font-weight: 700; font-size: 14px; display: block;">TS. Nguyễn Minh Châu</span>
      <small class="user-info" style="color: #64748b; font-size: 11px; display: block; margin-top: 2px;">Mã GV: GV001 • Khoa CNTT</small>
    </div>
    <!-- Avatar Cụm Clickable -->
    <div class="avatar" onclick="toggleUserDropdown(event)" style="background: #cbd5e1; width: 42px; height: 42px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; color: #334155; font-size: 16px; cursor: pointer; user-select: none; transition: transform 0.15s;" title="Bấm vào đây để mở menu cá nhân">GV</div>
    ...
    <div class="user-dropdown-menu" id="userDropdownMenu" style="display: none; position: absolute; right: 0; top: 52px; background: white; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); min-width: 200px; z-index: 10002; padding: 6px 0;">
      <div style="padding: 10px 16px; border-bottom: 1px solid #f1f5f9; font-size: 13px;">
        <strong style="color: #1e293b; display: block;">TS. Nguyễn Minh Châu</strong>
        <span style="color: #64748b; font-size: 11px;">Giảng viên • Khoa CNTT</span>
      </div>
...
<nav class="navbar">
  ...
  <a href="lich-giang-day.php" class="<?= $current_page == 'lich-giang-day.php' ? 'active' : '' ?>">LỊCH GIẢNG DẠY</a>
  <a href="diem.php" class="<?= $current_page == 'diem.php' ? 'active' : '' ?>">QUẢN LÝ LỚP HỌC</a>
</nav>
